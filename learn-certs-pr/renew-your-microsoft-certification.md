...
Ознакомьтесь со следующей дорожной картой (может быть изменена), поскольку мы первоначально запускаем аттестации для продления сертификатов, которые уже были на выпущены рынок.

| Аттестация для продления сертификации будет доступна в начале февраля 2021 г. |
| --- |
...
| Аттестация для продления сертификации будет доступна с середины февраля по март 2021 г. |
| --- |
...
15 декабря 2020 г. мы [объявили об изменении политики истечения срока сертификации](https://aka.ms/CertRenewalBlog). Начиная с июня 2021 года, любые сертификаты [на основе роли](/learn/certifications/browse/?type=role-based) или [на основе специальности](/learn/certifications/browse/?type=specialty) будут действительны в течении одного года с даты их получения. Любые ролевые и специальные сертификаты, полученные до июня 2021 года, будут действительны в течение двух лет с даты их получения.
...
2. **Обязательно ли мне подключать свой профиль сертификации к профилю Документов в Microsoft Learn?**
    ...
    Для оптимальной работы мы рекомендуем использовать одни и те же учетные данные для профиля сертификации и профиля Документов в Microsoft Learn. Обратите внимание, что к одному профилю Документов можно подключить только один профиль сертификации, и его нельзя будет изменить позже. Итак, если у вас есть несколько профилей сертификации, пожалуйста, [свяжитесь с нашей группой поддержки сертификации](https://aka.ms/mcpforum), чтобы объединить ваши профили сертификации Microsoft перед подключением к профилю Документов.
...
4. **Срок действия моего сертификата истек. Что мне делать сейчас?**

    Если срок действия вашего сертификата истекает, вы должны снова получить его, сдав требуемые экзамены.

5. **Могу ли я продлить свой сертификат, сдав бета-экзамен или пересдав экзамен до истечения срока действия моего сертификата?**

    Нет. Чтобы продлить свой сертификат, вам необходимо пройти аттестацию до истечения срока его действия.

6. **Все ли сертификации Microsoft имеют опцию продления?**
...
10. **Сколько раз я могу пересдавать аттестацию?**
...
11. **Если я продлеваю свой ролевой сертификат на уровне эксперта, и он подразумевает обязательную сертификацию на уровне младшего сотрудника, будет ли моя сертификация на уровне младшего сотрудника автоматически продлена?**

    Нет. Каждую сертификацию необходимо продлевать отдельно, проходя соответствующую аттестацию.

12. **Моя сертификация на уровне эксперта требует получения сертификата на уровне младшего специалиста в качестве предварительного условия. Если истечет срок сертификации на уровне младшего специалиста, не истечет ли автоматически мой экспертный уровень?**
